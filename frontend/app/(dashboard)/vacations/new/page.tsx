...
export default function NewVacationPage() {
  ...
  const selectedEmployee = useMemo(() => {
    ...
    return employees.data.find((e) => e.id === employeeId);
  }, [employeeId, employees?.data]);

  // Total de dias de férias por ano
  const TOTAL_VACATION_DAYS = 30;

  // Calcular quantidade de dias do período selecionado (dias que vai tirar agora)
  ...
  // Máximo de dias que pode vender (mínimo entre 10 e dias restantes)
  const maxSellableDays = useMemo(() => {
    return Math.min(10, remainingDays);
  }, [remainingDays]);

  // Saldo de dias após venda (dias restantes - dias vendidos)
  const balanceAfterSale = useMemo(() => {
    return Math.max(0, remainingDays - soldDays);
  }, [remainingDays, soldDays]);

  // Resetar cálculo quando dados mudarem
  useEffect(() => {
    setHasCalculated(false);
    setCalculation(null);
  }, [employeeId, startDate, endDate, soldDays, advance13thSalary]);
  ...
  const handleCalculate = () => {
    ...
    // Total de dias pagos = dias tirados + dias vendidos
    // O cálculo considera:
    // - vacationDays = dias que vai gozar (daysFromDates)
    // - soldDays = dias vendidos (dos restantes)
  ...
  const createMutation = useMutation({
    mutationFn: vacationApi.create,
    onSuccess: () => {
      toastSuccess('Férias cadastradas com sucesso');
      router.push('/vacations');
    },
    onError: (error) => toastErrorFromException(error, 'Erro ao cadastrar férias'),
  });
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Nova Férias"
        subtitle="Cadastre uma nova férias para um funcionário"
      />
          ...
          <SectionCard title="Dados da Férias">
            <div className="grid gap-4">
                ...
                {selectedEmployee && (
                  <p className="text-sm text-muted-foreground mt-1">
                    ...
                    {selectedEmployee.monthlySalary
                      ...
                      : <span className="text-yellow-600">Não informado</span>}
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                {soldDays > maxSellableDays && remainingDays > 0 && (
                  <p className="text-sm text-destructive mt-1">
                    Máximo de {maxSellableDays} dias podem ser vendidos
                  </p>
                )}
              </div>
              ...
              {/* Botão Calcular (antes de calcular) */}
              {!hasCalculated && (
                <Button
                  type="button"
            ...
            {!hasCalculated ? (
              <div className="flex flex-col items-center justify-center py-12 text-muted-foreground">
                ...
                <p className="text-sm text-center">
                  Preencha os dados do funcionário e período,
                  <br />
